...
class Profile extends MY_Controller 
{
	...
	public function index()
	{
		...
		$data['history']	= $this->profile->get_pending();
		
		$this->load->view('member/pages/nsm_profile', $data);
	}

	public function update($id)
	{
		$data['content'] = $this->profile->where('token', $id)->first();
		...
		if (!empty($_FILES) && $_FILES['image']['name'] !== '') {
			...
			if ($upload) {
				...
				redirect(base_url("member/profile/update/$id"));
			}
		}

		if (!$this->profile->validate()) {
			$data['title']			= 'Ubah Data Profile';
			$data['form_action']	= base_url("member/profile/update/$id");
			
		    $this->load->view('member/formulir/nsm_profile', $data);
			return;
		}


		if ($this->profile->where('token', $id)->update($data['input'])) {
			$this->session->set_flashdata('success', 'Your Profile has been successfully changes!');
		} else {
