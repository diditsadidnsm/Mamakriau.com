...
class User extends MY_Controller
{
	...
	public function index($page = null)
	{
		...
		$this->load->view('administrator/nsm_user');
	...
	public function search($page = null)
	{
		...
		$data['pagination']	= $this->user->makePagination(
			base_url('administrator/user/search'),
			3,
			$data['total_rows']
		);
		$this->load->view('administrator/material/nsm_header', $data);
	...
	public function create()
	{
		...
		if (!empty($_FILES) && $_FILES['image']['name'] !== '') {
			...
			if ($upload) {
				...
				redirect(base_url('administrator/user/create'));
			}
		}
		...
		if (!$this->user->validate()) {
			...
			$data['input']			= $input;
			$data['form_action']	= base_url('administrator/user/create');
	...
	public function edit($id)
	{
		...
		if (!empty($_FILES) && $_FILES['image']['name'] !== '') {
			...
			if ($upload) {
				...
				redirect(base_url("administrator/user/edit/$id"));
			}
		}

		if (!$this->user->validate()) {
			$data['title']			= 'Ubah Pengguna';
			$data['form_action']	= base_url("administrator/user/edit/$id");
